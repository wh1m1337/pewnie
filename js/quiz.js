...
const SK = { listening: ['Słuchanie', 'Аудіювання', 'ear'], reading: ['Czytanie', 'Читання', 'book'], grammar: ['Gramatyka', 'Граматика', 'cards'] };
...
function questions(source) {
  ...
  return {
    ...
    check() {
      if (chosen.some((c) => c < 0)) { toast('Відповісти треба на всі питання.'); return null; }
      locked = true;
      let ok = 0;
      blocks.forEach((b, i) => {
...
export async function quizView(skill, weekId, taskId) {
  ...
  if (skill === 'listening') {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!hasPolishVoice()) toast('Польського голосу в системі немає — додай його в налаштуваннях ОС (Мова → Голоси).', 4500);
      if (playing) { playing.stop(); playing = null; btn.replaceChildren(icon('play', 18), 'Слухати запис'); return; }
      plays++;
      counter.textContent = `Прослухано: ${plays} ${plays > 2 ? '(більше, ніж на іспиті)' : 'з 2 (на іспиті — двічі)'}`;
      btn.replaceChildren(icon('stop', 18), 'Зупинити');
      playing = speak(items, { rate: task.rate ?? (level === 'B2' ? 1 : 0.92) * store.get().rate / 0.9, onDone: () => { playing = null; btn.replaceChildren(icon('play', 18), 'Слухати ще раз'); } });
    });
    const script = h('details', { class: 'model' }, h('summary', null, icon('eye', 18), 'Транскрипт', h('span', { class: 'hint' }, ' — відкрий після відповідей')),
      h('div', { class: 'transcript-view' }, (task.lines || [{ t: task.script }]).map((l) => h('p', null, l.who && h('b', null, `${l.who}: `), l.t))));
    head = h('div', { class: 'card listen' }, h('div', { class: 'eyebrow' }, task.intro || 'Прослухай запис і відповідай на питання'), h('div', { class: 'row' }, btn, counter), script);
  } else if (skill === 'reading') {
    head = h('article', { class: 'card reading' }, h('h2', { class: 'read-title' }, task.textTitle || task.title), task.text.split('\n\n').map((p) => h('p', null, p)));
  } else {
    head = h('div', { class: 'note note--yellow' }, h('div', { class: 'eyebrow' }, 'Постав правильну форму'), h('p', null, task.intro || 'Вибери варіант, який пасує граматично.'));
  }
  ...
  const checkBtn = h('button', { class: 'btn btn--red', type: 'button', onclick: () => {
    ...
    clear(out).append(h('div', { class: 'report-head' }, circledGrade(gradeFromScore(score)), h('div', null, h('h3', null, `${score}% правильно`), h('p', { class: 'hint' }, score >= 75 ? 'Добре! Перечитай пояснення до помилок — вони найцінніші.' : 'Не біда: розбери пояснення й спробуй ще раз через день-два.'))),
      h('div', { class: 'row' }, h('button', { class: 'btn btn--ghost', type: 'button', onclick: () => { qs.reset(); clear(out); } }, icon('redo', 16), 'Ще раз'), h('a', { class: 'btn btn--ghost', href: `#/week/${weekId}` }, 'До тижня', icon('arrow', 16))));
    out.scrollIntoView({ behavior: 'smooth', block: 'center' });
  } }, icon('check', 18), 'Перевірити');

  return h('div', { class: 'view narrow' }, backTo(`#/week/${weekId}`, `Тиждень · ${week.theme.pl}`),
...
export async function vocabView() {
  ...
  function card() {
    clear(stage);
    if (!queue.length) return stage.append(h('div', { class: 'note' }, h('h3', null, 'На сьогодні все!'), h('p', null, `Вивчено ${learned} з ${all.length}. Нові слова — у понеділок, а повторення з’являться за розкладом.`)));
    if (i >= queue.length) {
      return stage.append(h('div', { class: 'note note--green' }, h('h3', null, 'Сесію завершено'), h('p', null, `Знав(ла) ${right} з ${queue.length}. Слова, які «не пішли», повернуться завтра.`), h('a', { class: 'btn', href: '#/' }, 'На головну')));
    }
    const v = queue[i];
    let flipped = false;
    ...
    const actions = h('div', { class: 'row', hidden: true }, h('button', { class: 'btn btn--ghost', type: 'button', onclick: () => next(false) }, icon('x', 16), 'Ще не знаю'), h('button', { class: 'btn', type: 'button', onclick: () => next(true) }, icon('check', 16), 'Знаю'));
    const face = h('button', { class: 'vcard', type: 'button', onclick: () => { if (flipped) return; flipped = true; back.hidden = false; actions.hidden = false; face.classList.add('flipped'); } },
      h('span', { class: 'eyebrow' }, `${i + 1} / ${queue.length} · ${v.theme}`), h('span', { class: 'vpl' }, v.pl), h('span', { class: 'hint' }, flipped ? '' : 'натисни, щоб побачити переклад'));
    stage.append(h('div', { class: 'vwrap' }, face, h('div', { class: 'vspeak' }, speakBtn(v.pl, { label: 'Вимова' })), back, actions));
  ...
  return h('div', { class: 'view narrow' }, backTo('#/', 'Головна'),
    pageHead({ eyebrow: 'Słownictwo · картки', title: 'Слова', sub: `${all.length} слів відкрито · ${learned} вивчено · ${due.length} на повторення`, actions: levelSwitch() }), stage);
}
