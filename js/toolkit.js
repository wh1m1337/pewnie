...
const TABS = [['exam', tx('Формат іспиту')], ['strategy', tx('Стратегії')], ['connectors', tx('Зв’язки')], ['letters', tx('Шаблони листів')], ['phrases', tx('Фрази для усної')], ['friends', tx('Хибні друзі')], ['errors', tx('Типові помилки')]];
...
export async function toolkitView(tab = 'exam') {
  ...
  const nav = h('nav', { class: 'tabs', 'aria-label': tx('Розділи довідника') }, TABS.map(([k, l]) => h('a', { class: `tab ${k === t ? 'on' : ''}`, href: `#/toolkit/${k}` }, l)));
  let body;
  ...
  if (t === 'exam') {
    ...
    body = h('div', { class: 'tk' },
      ...
      h('h2', null, tx('Структура іспиту {0}', level)),
      ...
      h('h2', null, tx('Усна частина: три завдання')),
      h('div', { class: 'grid3' }, L.speaking.map((s) => h('div', { class: 'card' }, h('h3', null, s.name), h('p', null, rich(s.what)), h('p', { class: 'hint' }, s.tip)))),
      h('h2', null, tx('Письмо: два тексти')),
      h('div', { class: 'grid3' }, L.writing.map((s) => h('div', { class: 'card' }, h('h3', null, s.name), h('p', null, rich(s.what)), h('p', { class: 'hint' }, s.tip)))),
      h('h2', null, tx('За чим оцінюють')),
      h('div', { class: 'grid2' }, ['speaking', 'writing'].map((k) => h('div', { class: 'card' }, h('h3', null, k === 'speaking' ? tx('Усна відповідь') : tx('Письмо')), h('ul', { class: 'flist' }, tk.exam.criteria[k].map((c) => h('li', null, h('span', null, h('strong', null, c.name), ' — ', c.ua))))))));
  } else if (t === 'strategy') {
    body = h('div', { class: 'tk grid2' }, tk.strategies.map((s) => h('article', { class: 'card' }, h('div', { class: 'eyebrow' }, s.skill === 'speaking' ? tx('Мовлення') : s.skill === 'writing' ? tx('Письмо') : tx('Загальне')), h('h3', null, s.title), h('p', null, rich(s.body)), s.steps && h('ol', { class: 'plan' }, s.steps.map((x) => h('li', null, rich(x)))))));
    ...
    body = h('div', { class: 'tk' }, h('p', { class: 'lead sm' }, tx('Слова, схожі з українськими, але з іншим значенням. Найчастіші пастки на іспиті.')),
    ...
    body = h('div', { class: 'tk' }, h('p', { class: 'lead sm' }, tx('Помилки, які роблять саме україномовні. Перевіряй свої тексти на них перед здачею.')),
  ...
  return h('div', { class: 'view' }, pageHead({ eyebrow: tx('Довідник'), title: tx('Шпаргалка без води'), sub: tx('Усе, що треба тримати під рукою під час підготовки.'), actions: t === 'exam' ? levelSwitch() : null }), nav, body);
}
